...
/* ── Responsive nav: collapse tool links into dropdown ──────── */
#lrt-tools-dropdown { display: none; position: relative; }
...
#lrt-tools-btn {
  ...
  color: #3D3D3D !important; padding: 5px 10px !important;
  border-radius: 4px !important; cursor: pointer !important;
  transition: background .15s, color .15s !important;
  white-space: nowrap !important;
}
#lrt-tools-btn:hover            { background: #F5F4F0 !important; color: #111111 !important; }
#lrt-tools-btn.lrt-current      { background: #F5F4F0 !important; color: #111111 !important; font-weight: 600 !important; }
body.dark #lrt-tools-btn        { color: #AAAAAA !important; }
body.dark #lrt-tools-btn:hover  { background: #1A1A1A !important; color: #EEEEEE !important; }
body.dark #lrt-tools-btn.lrt-current { background: #1A1A1A !important; color: #EEEEEE !important; }

#lrt-tools-btn svg { width: 12px; height: 12px; display: block !important; pointer-events: none; transition: transform .15s; flex-shrink: 0; }
#lrt-tools-btn[aria-expanded="true"] svg { transform: rotate(180deg); }
...
#lrt-tools-menu {
  ...
  min-width: 148px; z-index: 99999;
  box-shadow: 0 4px 16px rgba(0,0,0,0.08);
  flex-direction: column; gap: 2px;
}
#lrt-tools-menu.lrt-open { display: flex; }
body.dark #lrt-tools-menu { background: #1A1A1A; border-color: #333333; box-shadow: 0 4px 16px rgba(0,0,0,0.3); }
...
#lrt-tools-menu a:hover              { background: #F5F4F0; color: #111111; }
#lrt-tools-menu a.lrt-current        { background: #F5F4F0; color: #111111; font-weight: 600; }
body.dark #lrt-tools-menu a          { color: #AAAAAA; }
body.dark #lrt-tools-menu a:hover    { background: #242424; color: #EEEEEE; }
body.dark #lrt-tools-menu a.lrt-current { background: #242424; color: #EEEEEE; }

@media (max-width: 680px) {
  #lrt-nav-links       { display: none !important; }
  #lrt-tools-dropdown  { display: block; }
}
...
function buildNavbar(currentId) {
  ...
  // Narrow dropdown: menu items + button label
  const dropLinks = NAV_TOOLS.map(t =>
    `<a href="${t.href}"${t.id === currentId ? ' class="lrt-current"' : ''}>${t.label}</a>`
  ).join('\n          ');
  const currentTool = NAV_TOOLS.find(t => t.id === currentId);
  const btnLabel = currentTool ? currentTool.label : 'Tools';
  const btnClass = currentTool ? 'lrt-current' : '';
...
<nav id="lrt-nav" aria-label="lawreview.tools">
  <div id="lrt-nav-inner">
    ...
    <div id="lrt-nav-right">
      ...
      <!-- Narrow: Tools dropdown -->
      <div id="lrt-tools-dropdown">
        <button id="lrt-tools-btn"${btnClass ? ` class="${btnClass}"` : ''} aria-expanded="false" aria-haspopup="true">
          ${btnLabel}${CHEVRON_SVG}
        </button>
        <div id="lrt-tools-menu" role="menu">
          ${dropLinks}
        </div>
      </div>
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var toolsBtn  = document.getElementById('lrt-tools-btn');
    var toolsMenu = document.getElementById('lrt-tools-menu');
    if (toolsBtn && toolsMenu) {
      toolsBtn.addEventListener('click', function (e) {
        e.stopPropagation();
        var open = toolsMenu.classList.toggle('lrt-open');
        toolsBtn.setAttribute('aria-expanded', open);
      });
      document.addEventListener('click', function () {
        toolsMenu.classList.remove('lrt-open');
        toolsBtn.setAttribute('aria-expanded', 'false');
      });
      toolsMenu.addEventListener('click', function (e) {
        e.stopPropagation(); // keep open if clicking a link (browser navigates anyway)
      });
    }
  });
}());
