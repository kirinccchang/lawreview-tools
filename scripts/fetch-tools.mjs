...
#lrt-nav-links { display: flex !important; align-items: center !important; gap: 6px !important; }

/* ── Responsive nav: hamburger ──────────────────────────────── */
#lrt-hamburger { display: none; position: relative; }
...
#lrt-hamburger-btn {
  ...
  color: #3D3D3D !important; flex-shrink: 0 !important;
  transition: background .15s, color .15s !important;
}
#lrt-hamburger-btn:hover         { background: #F5F4F0 !important; color: #111111 !important; }
body.dark #lrt-hamburger-btn     { color: #AAAAAA !important; }
body.dark #lrt-hamburger-btn:hover { background: #1A1A1A !important; color: #EEEEEE !important; }
#lrt-hamburger-btn svg { width: 18px !important; height: 18px !important; display: block !important; pointer-events: none; }

#lrt-hamburger-menu {
  display: none;
  position: absolute; top: calc(100% + 6px); right: 0;
  background: #FFFFFF; border: 1px solid #D0CEC9;
  border-radius: 8px; padding: 4px;
  min-width: 180px; z-index: 99999;
  box-shadow: 0 4px 20px rgba(0,0,0,0.12);
  flex-direction: column; gap: 1px;
}
#lrt-hamburger-menu.lrt-open { display: flex; }
body.dark #lrt-hamburger-menu { background: #1A1A1A; border-color: #333333; box-shadow: 0 4px 20px rgba(0,0,0,0.4); }

#lrt-hamburger-menu a,
#lrt-hamburger-menu #lrt-hm-theme-btn {
  ...
  padding: 7px 10px !important; border-radius: 5px !important;
  transition: background .12s, color .12s !important;
  white-space: nowrap !important; width: 100% !important; cursor: pointer !important;
}
#lrt-hamburger-menu a:hover,
#lrt-hamburger-menu #lrt-hm-theme-btn:hover { background: #F5F4F0 !important; color: #111111 !important; }
#lrt-hamburger-menu a.lrt-current { background: #F5F4F0 !important; color: #111111 !important; font-weight: 600 !important; }
body.dark #lrt-hamburger-menu a,
body.dark #lrt-hamburger-menu #lrt-hm-theme-btn { color: #AAAAAA !important; }
body.dark #lrt-hamburger-menu a:hover,
body.dark #lrt-hamburger-menu #lrt-hm-theme-btn:hover { background: #242424 !important; color: #EEEEEE !important; }
body.dark #lrt-hamburger-menu a.lrt-current { background: #242424 !important; color: #EEEEEE !important; }
#lrt-hamburger-menu svg { width: 14px !important; height: 14px !important; display: block !important; flex-shrink: 0 !important; pointer-events: none; }
...
body.dark .subtitle { color: #AAAAAA !important; }
...
const HAMBURGER_SVG = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg>`;
const MOON_SVG_SM   = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>`;
const GH_SVG_SM     = `<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0 0 24 12c0-6.63-5.37-12-12-12z"/></svg>`;

function buildNavbar(currentId) {
  // Wide: individual links
  const wideLinks = NAV_TOOLS.map(t =>
    `<a href="${t.href}" class="lrt-nav-link${t.id === currentId ? ' lrt-current' : ''}">${t.label}</a>`
  ).join('\n        ');

  // Hamburger menu: tool links + divider + GitHub + theme toggle
  const menuLinks = NAV_TOOLS.map(t =>
    `<a href="${t.href}"${t.id === currentId ? ' class="lrt-current"' : ''}>${t.label}</a>`
  ).join('\n          ');
...
<nav id="lrt-nav" aria-label="lawreview.tools">
  <div id="lrt-nav-inner">
    ...
    <div id="lrt-nav-right">
      ...
      <!-- Narrow: hamburger -->
      <div id="lrt-hamburger">
        <button id="lrt-hamburger-btn" aria-label="Open menu" aria-expanded="false" aria-haspopup="true">
          ${HAMBURGER_SVG}
        </button>
        <div id="lrt-hamburger-menu" role="menu">
          ${menuLinks}
          <div class="lrt-menu-divider" role="separator"></div>
          <a href="https://github.com/kirinccchang" target="_blank" rel="noopener noreferrer">
            ${GH_SVG_SM}GitHub
          </a>
          <button id="lrt-hm-theme-btn">
            ${MOON_SVG_SM}<span id="lrt-hm-theme-label">Dark mode</span>
          </button>
        </div>
      </div>
    </div>
...
<footer id="lrt-footer">
  <div id="lrt-footer-inner">
    ...
    <div id="lrt-footer-bottom">
      ...
      <div style="margin-top:6px; font-size:11px; opacity:0.6;">Not affiliated with or endorsed by The Bluebook, Zotero, or perma.cc.</div>
    </div>
  </div>
</footer>
<!-- ══ end footer ═══════════════════════════════════════════ -->`.trim();
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // Hamburger toggle
    var hBtn  = document.getElementById('lrt-hamburger-btn');
    var hMenu = document.getElementById('lrt-hamburger-menu');
    if (hBtn && hMenu) {
      hBtn.addEventListener('click', function (e) {
        e.stopPropagation();
        var open = hMenu.classList.toggle('lrt-open');
        hBtn.setAttribute('aria-expanded', String(open));
      });
      document.addEventListener('click', function () {
        hMenu.classList.remove('lrt-open');
        hBtn.setAttribute('aria-expanded', 'false');
      });
    }

    // Theme toggle inside hamburger
    var hmBtn   = document.getElementById('lrt-hm-theme-btn');
    var hmLabel = document.getElementById('lrt-hm-theme-label');
    function syncHmLabel() {
      if (hmLabel) hmLabel.textContent = document.body.classList.contains('dark') ? 'Light mode' : 'Dark mode';
    }
    syncHmLabel();
    if (hmBtn) {
    ...
    new MutationObserver(syncHmLabel).observe(document.body, { attributes: true, attributeFilter: ['class'] });
  });
}());
<\/script>`.trim();
